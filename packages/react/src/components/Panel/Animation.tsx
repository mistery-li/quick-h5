...
const Animation = ({ visible, onClose }) => {
  ...
  const onAnimation = async (animation, event: React.MouseEvent) => {
    ...
    await runAnimation(event.target, [animation] as any)
    // 防止无限触发同一元素的动画
    //   setTimeout(() => {
          ...
          return (
            ...
            >
              {animation.children.map((item) => (
                <div
                  onMouseEnter={(event) => onAnimation(item, event)}
                  className="m-1 p-4 text-center bg-gray-200 flex-none w-24 overflow-auto"
                >
                  {item.label}
                </div>
              ))}
