...
export const useMarkerLine = (direction) => {
  const target = useSelector((state: RootState) => getCurComp(state))

  const compares = useSelector((state: RootState) => getComps(state)).filter(
    (comp) => comp.uuid !== target?.uuid
  )

  console.log([compares, target], 'compares')
  ...
  const calcLineValues = (pos, target, compare, linekey) => {
    console.log([target, compare], 'calcLineValues')
    const { x, y } = pos
  ...
  const calcPosValuesSingle = (pos, direction, target, compare, lineKey) => {
    ...
    if (result.origin !== result.origin || result.length !== result.length) {
      console.log(result, 'nan result result calcPosValuesSingle')
    }
    return result
  }
  ...
  useEffect(() => {
    if (direction && compares.length > 0) {
      console.log([direction, target, compares], 'direction')
